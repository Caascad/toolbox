...
def get_packages_releases(token, pkgs):
    ...
    for pkg in pkgs:
        ...
        # query += pkgs[pkg]['alias'] + ':repository(owner:"' + pkgs[pkg]['owner'] + '",name:"' + pkgs[pkg][
        #     'repo'] + '"){refs(refPrefix:"refs/tags/",last:1){nodes{name}}}'
    query += '}'
...
def update_package(token, pkgs, pkg, version):
    ...
    if version == NO_VERSION:
        print('\033[96m', pkg, '\033[0mwill be updated to the latest commit')
    else:
        cmd += ['-v', version]
        print(
            '\033[96m', pkg, '\033[0mis upgradable from version\033[92m', pkgs[pkg][
...
def main():
    ...
    for pkg in pkgs:
        ...
        #nodes = result['data'][pkgs[pkg]['alias']]['refs']['nodes']
        if len(nodes) > 0:
            ...
            if pkgs[pkg]['c_version'] != pkgs[pkg]['o_version']:
                ...
                print('\033[96m', pkg, '\033[0mis up to date. Current version:\033[92m', pkgs[pkg]['o_version'], '\033[0m')
