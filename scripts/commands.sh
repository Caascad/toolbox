...
make-shell() {
    ...
    for tool in "$@"
    do
        instantiate_args="$instantiate_args -A $tool"
    done
    result=$(nix-instantiate $url $instantiate_args 2>&1 || true)
    if echo $result | grep -q error; then
        log-error "Error: '$(echo $result | sed "s/.*attribute '\([^']*\)'.*/\1/")' is not available in the toolbox"
        exit 1
    fi
